<?php
  ...
  $sql = "SELECT * FROM `called_numbers` WHERE `user_id`=:user_id ORDER BY `numbers`";
  $stmt = $db->prepare($sql);
  ...
  if (isset($_POST['save_phone'])) {
    ...
    $sql = "SELECT * FROM `called_numbers` WHERE `calling_code`=:calling_code AND `prefix`=:prefix AND `numbers`=:numbers AND `user_id`=:user_id";
    $stmt = $db->prepare($sql);
    ...
    if (count($errors) == 0) {
      ...
      $sql = "INSERT INTO `called_numbers` (`calling_code`, `prefix`, `numbers`, `user_id`) VALUES (?, ?, ?, ?)";
      $stmt = $db->prepare($sql);
